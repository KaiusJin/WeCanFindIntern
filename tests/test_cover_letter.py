...
def test_cover_letter_runs_writer_and_reviewer_ai(monkeypatch):
    responses = iter(
        [
            ...
            '{"approved":true,"summary":"All candidate claims are supported.","issues":[],"unsupported_claims":[]}',
        ]
    )
    calls = []

    def fake_gemini(api_key, prompt, requested_model):
        calls.append(prompt)
        return next(responses)

    monkeypatch.setattr(cover_letter_service, "call_gemini", fake_gemini)
    result = cover_letter_service.generate_cover_letter(
        ...
        user_info=UserProfile(
            full_name="Jane Doe", email="jane@example.com", phone="555-0100", linkedin="linkedin.com/in/jane"
        ),
        provider="Gemini",
        model_name="gemini-test",
        api_key="AIzaSy_fake",
...
def test_cover_letter_retries_writer_after_reviewer_rejection(monkeypatch):
    responses = iter(
        [
            ...
            '{"approved":false,"summary":"Unsupported claim found.","issues":["Remove unsupported claim."],"unsupported_claims":["Claim X"]}',
            '{"cover_letter":"draft two","hr_info":{}}',
            '{"approved":true,"summary":"Grounded after revision.","issues":[],"unsupported_claims":[]}',
        ]
    )
    calls = []
