...
> 本文件保留审计证据和风险决策；它不是运行时规范的唯一来源。当前架构和失败/恢复语义以 [技术总文档](TECHNICAL_DOCUMENTATION.md)、[可靠性与恢复手册](RELIABILITY_AND_RECOVERY.md) 以及代码为准。涉及旧目录结构或“计划内修复”的段落应视为审计时点记录，不应覆盖当前实现。
