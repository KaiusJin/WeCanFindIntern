...
来源字段必须先通过周期与金额一致性校验：hourly `5–500`、daily `40–5,000`、
weekly `100–25,000`、monthly `500–100,000`、yearly `5,000–2,000,000`。
JobSpy 从 JD 推断的薪资不视为 provider structured data，统一由平台正则与 DeepSeek
流水线重新解析。前端统一换算并显示时薪；异常或无法确认的值显示 `Salary not disclosed`。
