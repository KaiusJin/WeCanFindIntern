...
BATCH_ITEM_SCHEMA = (
    '{"job": <1-based index>, "found": <bool>, "season": <"winter"|"spring"|"summer"|"fall"|null>,'
    ' "year": <int|null>, "confidence": <0-1>, "evidence": <string|null>}'
)
        ...
        json.dumps(
            {
                ...
                "properties": {
                    "results": {
                        "type": "array",
                        "items": json.loads(BATCH_ITEM_SCHEMA.replace("<1-based index>", "1")
                                            .replace("<bool>", "true")),
                    }
                },
                "required": ["results"],
    ...
    try:
        ...
        for item in raw_results:
            if not isinstance(item, dict):
                continue
            try:
                job_index = int(item.pop("job"))
            except (TypeError, ValueError, KeyError):
                continue
            try:
                parsed_items[job_index] = RecruitingTermLLMResult.model_validate(item)
